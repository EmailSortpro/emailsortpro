    ...
    try {
        ...
        return `
            <div class="keywords-tab-layout">
                ...
                <!-- Grille des catégories en carrés -->
                <div class="categories-square-grid">
                    ${Object.entries(categories).map(([id, category]) => {
                        const isCustom = category.isCustom || customCategories[id];
                        const isActive = !activeCategories || activeCategories.includes(id);
                        
                        return `
                            <div class="category-square ${!isActive ? 'inactive' : ''}" data-category-id="${id}">
                                ...
                                ${isCustom ? '<span class="category-badge custom">Perso</span>' : ''}
                                <button class="category-edit-btn" onclick="window.categoriesPage.openCategoryMenu('${id}')" title="Modifier">
...
addStyles() {
    ...
    styles.textContent = `
        ...
        /* ===== GRILLE DE CATÉGORIES EN CARRÉS ===== */
        .keywords-tab-layout {
            display: flex;
            flex-direction: column;
            gap: 20px;
        ...
        .category-square.inactive {
            opacity: 0.5;
            background: #f9fafb;
        }
        
        .category-square.inactive:hover {
            opacity: 0.7;
        }
        ...
        .category-square:hover .category-edit-btn {
            opacity: 1;
        }
    ...
    openCategoryMenu(categoryId, event) {
    if (event) {
    ...
    const isActive = window.categoryManager?.isCategoryActive(categoryId);
    
    const menuHTML = `
        <div id="category-context-menu" class="category-context-menu">
            <button class="context-menu-item" onclick="window.categoriesPage.openKeywordsModal('${categoryId}'); window.categoriesPage.closeContextMenu();">
                <i class="fas fa-key"></i>
                <span>Gérer les mots-clés</span>
            </button>
            <button class="context-menu-item" onclick="window.categoriesPage.openCategoryFiltersModal('${categoryId}'); window.categoriesPage.closeContextMenu();">
                <i class="fas fa-filter"></i>
                <span>Gérer les filtres</span>
            </button>
            <div class="context-menu-separator"></div>
            <button class="context-menu-item" onclick="window.categoriesPage.toggleCategoryActive('${categoryId}'); window.categoriesPage.closeContextMenu();">
                <i class="fas ${isActive ? 'fa-eye-slash' : 'fa-eye'}"></i>
                <span>${isActive ? 'Désactiver' : 'Activer'}</span>
            </button>
            ${isCustom ? `
                <div class="context-menu-separator"></div>
                <button class="context-menu-item" onclick="window.categoriesPage.editCustomCategory('${categoryId}'); window.categoriesPage.closeContextMenu();">
                    <i class="fas fa-edit"></i>
                    <span>Modifier</span>
                </button>
                <button class="context-menu-item danger" onclick="window.categoriesPage.deleteCustomCategory('${categoryId}'); window.categoriesPage.closeContextMenu();">
    ...
    const button = event?.target?.closest('.category-edit-btn') || document.querySelector(`[data-category-id="${categoryId}"] .category-edit-btn`);
...
console.log('✅ CategoriesPage v9.0 loaded - Gestion complète des catégories et mots-clés');// CategoriesPage.js - Version 9.0 - Gestion complète des catégories et mots-clés
