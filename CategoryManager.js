// CategoryManager.js - Version 20.1 - Gestion robuste catégories avec priorité Gmail
...
class CategoryManager {
    constructor() {
        ...
        // NOUVEAU: Système de synchronisation renforcé Gmail/Outlook
        this.syncQueue = [];
        ...
        console.log('[CategoryManager] ✅ Version 20.1 - Gestion robuste Gmail/Outlook');
    }

    // ================================================
    // SYSTÈME DE SYNCHRONISATION AUTOMATIQUE RENFORCÉ
    // ================================================
    startAutoSync() {
        ...
        this.setupProviderSync();
    }

    setupProviderSync() {
        // Écouter les événements de MailService
        window.addEventListener('mailServiceInitialized', (event) => {
            const { provider } = event.detail;
            console.log(`[CategoryManager] 📧 MailService initialisé avec provider: ${provider}`);
            
            // Adapter les paramètres si nécessaire
            this.adaptToProvider(provider);
        });
    }

    adaptToProvider(provider) {
        console.log(`[CategoryManager] 🔧 Adaptation pour provider: ${provider}`);
        
        // Ajuster les mots-clés selon le provider
        if (provider === 'google') {
            this.optimizeForGmail();
        } else if (provider === 'microsoft') {
            this.optimizeForOutlook();
        }
    ...
    optimizeForGmail() {
        console.log('[CategoryManager] 📧 Optimisation pour Gmail');
        
        // Ajouter des mots-clés spécifiques Gmail si pas déjà présents
        if (this.weightedKeywords.notifications) {
            const gmailKeywords = ['gmail', 'google', 'noreply@gmail.com'];
            gmailKeywords.forEach(keyword => {
                if (!this.weightedKeywords.notifications.weak.includes(keyword)) {
                    this.weightedKeywords.notifications.weak.push(keyword);
                }
            });
        }
        
        // Optimiser les catégories marketing pour Gmail
        if (this.weightedKeywords.marketing_news) {
            const gmailMarketing = ['unsubscribe from this list', 'manage your subscriptions'];
            gmailMarketing.forEach(keyword => {
                if (!this.weightedKeywords.marketing_news.absolute.includes(keyword)) {
                    this.weightedKeywords.marketing_news.absolute.push(keyword);
                }
            });
        }
    }
    ...
    optimizeForOutlook() {
        console.log('[CategoryManager] 📧 Optimisation pour Outlook');
        
        // Ajouter des mots-clés spécifiques Outlook
        if (this.weightedKeywords.notifications) {
            const outlookKeywords = ['outlook', 'microsoft', 'office365'];
            outlookKeywords.forEach(keyword => {
                if (!this.weightedKeywords.notifications.weak.includes(keyword)) {
                    this.weightedKeywords.notifications.weak.push(keyword);
                }
            });
    ...
    notifySpecificModules(type, value) {
        ...
        // MailService avec notification provider
        if (window.mailService) {
            console.log('[CategoryManager] → MailService: changement catégories');
            // Le MailService peut ajuster ses paramètres selon les catégories
        }
    ...
    getCurrentProvider() {
        if (window.mailService && window.mailService.provider) {
            return window.mailService.provider;
        }
    ...
    loadCustomCategories() {
        try {
            ...
            Object.entries(this.customCategories).forEach(([id, category]) => {
                ...
                // Tracker les catégories vides SANS les supprimer
                if (totalKeywords === 0) {
    ...
    updateCustomCategory(categoryId, updates) {
        ...
        if (updates.keywords) {
            ...
            // Mettre à jour le tracker des catégories vides
    ...
    // ================================================
    // INITIALISATION DES CATÉGORIES OPTIMISÉE DUAL PROVIDER
    // ================================================
    initializeCategories() {
        this.categories = {
            // PRIORITÉ MAXIMALE - MARKETING & NEWS (Compatible Gmail/Outlook)
            marketing_news: {
                name: 'Marketing & News',
                icon: '📰',
                color: '#8b5cf6',
    ...
    initializeWeightedDetection() {
        this.weightedKeywords = {
            // PRIORITÉ MAXIMALE - MARKETING & NEWS (Optimisé Gmail/Outlook)
            marketing_news: {
                absolute: [
                    // Français
                    'se désinscrire', 'se desinscrire', 'désinscrire', 'desinscrire',
                    'désabonner', 'desabonner', 'ne plus recevoir',
                    'gérer vos préférences', 'gérer la réception', 'préférences email',
                    'paramétrez vos choix', 'vous ne souhaitez plus recevoir',
                    // Anglais (Gmail)
                    'unsubscribe', 'opt out', 'opt-out', 'stop emails',
                    'email preferences', 'manage your subscription', 'manage subscriptions',
                    ...
                    // Promotionnel
                    'newsletter', 'mailing list', 'mailing', 'campaign',
                    'limited offer', 'offre limitée', 'special offer', 'promotion',
                    'vente privée', 'soldes', 'shopping', 'boutique'
                ],
                strong: [
                    'promo', 'deal', 'offer', 'sale', 'discount', 'réduction',
                    'exclusive', 'special', 'limited', 'new', 'nouveau',
                    'acheter', 'commander', 'offre', 'remise', 'solde',
            ...
            security: {
                absolute: [
                    // Gmail/Google spécifique
                    'google security alert', 'gmail security', 'google account',
                    'suspicious activity on your google account',
                    'verify your google account', 'google verification',
                    // Outlook/Microsoft spécifique
                    'microsoft security', 'outlook security', 'office 365 security',
                    'suspicious activity on your microsoft account',
                ...
                strong: [
                    ...
                    'google', 'microsoft', 'gmail', 'outlook'
                ],
            ...
            meetings: {
                absolute: [
                    // Gmail/Google Meet
                    'google meet', 'google calendar', 'calendar invitation from google',
                    'meet.google.com', 'calendar.google.com',
                    // Outlook/Teams
                    'microsoft teams', 'teams meeting', 'outlook calendar',
                    'calendar invitation from outlook', 'office 365 calendar',
                    // Général
                    'demande de réunion', 'meeting request', 'réunion',
                    'schedule a meeting', 'planifier une réunion',
                    'invitation réunion', 'meeting invitation',
                    'zoom meeting', 'rendez-vous', 'appointment', 'rdv'
                ],
                strong: [
                    'meeting', 'réunion', 'schedule', 'planifier',
                    'calendar', 'calendrier', 'appointment', 'agenda',
                    'conférence', 'conference', 'call', 'teams', 'zoom'
                ],
            ...
            notifications: {
                absolute: [
                    // Gmail spécifique
                    'gmail-noreply@googlemail.com', 'noreply@gmail.com',
                    'no-reply@accounts.google.com', 'google-noreply@google.com',
                    // Outlook spécifique  
                    'noreply@email.microsoftonline.com', 'no-reply@outlook.com',
        ...
        console.log('[CategoryManager] Mots-clés par défaut initialisés (Gmail/Outlook) pour', Object.keys(this.weightedKeywords).length, 'catégories');
    }

    // ================================================
    // ANALYSE EMAIL OPTIMISÉE DUAL PROVIDER
    // ================================================
    analyzeEmail(email) {
    ...
    analyzeAllCategories(content) {
        ...
        // Construire la liste des catégories à analyser en EXCLUANT les vides
        ...
        const validCategoriesToAnalyze = [...allCategoriesToAnalyze].filter(categoryId => {
            // Skip immédiatement si catégorie trackée comme vide
    ...
    // Helper optimisé pour vérifier si les mots-clés sont vides
    ...
    selectByPriorityWithThreshold(results) {
        // Seuils pour avoir des emails "other"
    ...
    // ================================================
    // MÉTHODES UTILITAIRES ET HELPERS - OPTIMISÉES DUAL PROVIDER
    // ================================================
    ...
    calculateScore(content, keywords, categoryId) {
        ...
        // Bonus de base pour certaines catégories selon le provider
        const provider = this.getCurrentProvider();
        const categoryBonus = this.getProviderCategoryBonus(categoryId, provider);
        
        if (categoryBonus > 0) {
            totalScore += categoryBonus;
            matches.push({ keyword: 'provider_category_bonus', type: 'bonus', score: categoryBonus });
        }
        ...
        // Appliquer bonus de domaine selon le provider
        this.applyProviderDomainBonus(content, categoryId, matches, totalScore, provider);
    ...
    getProviderCategoryBonus(categoryId, provider) {
        const bonuses = {
            google: {
                'security': 15, // Bonus pour sécurité Google
                'notifications': 10, // Gmail notifications
                'meetings': 10 // Google Meet
            },
            microsoft: {
                'security': 15, // Bonus pour sécurité Microsoft
                'notifications': 10, // Outlook notifications
                'meetings': 10 // Teams
            }
        };
        
        return bonuses[provider]?.[categoryId] || 0;
    }

    applyProviderDomainBonus(content, categoryId, matches, totalScore, provider) {
        const providerDomainBonuses = {
            google: {
                security: ['google', 'gmail', 'googlemail', 'accounts.google'],
                notifications: ['gmail', 'google', 'noreply@gmail', 'googlemail'],
                meetings: ['meet.google', 'calendar.google', 'google.com']
            },
            microsoft: {
                security: ['microsoft', 'outlook', 'office365', 'microsoftonline'],
                notifications: ['outlook', 'microsoft', 'office365', 'microsoftonline'],
                meetings: ['teams.microsoft', 'outlook', 'office365']
            }
        };
        
        const domainKeywords = providerDomainBonuses[provider]?.[categoryId] || [];
        
        for (const domainKeyword of domainKeywords) {
            if (content.domain.includes(domainKeyword) || content.text.includes(domainKeyword)) {
                const bonus = 40;
                totalScore += bonus;
                matches.push({ keyword: `${domainKeyword}_${provider}_domain`, type: 'provider_domain', score: bonus });
                break; // Un seul bonus par catégorie
            }
        }
        
        // Bonus génériques
        const genericDomainBonuses = {
            security: ['security', 'auth', '2fa', 'verification'],
        ...
        if (genericDomainBonuses[categoryId]) {
            for (const domainKeyword of genericDomainBonuses[categoryId]) {
    ...
    // ================================================
    // MÉTHODES UTILITAIRES DE CONTENU - OPTIMISÉES
    // ================================================
    ...
    extractCompleteContent(email) {
        ...
        return {
            ...
            provider: email.provider || this.getCurrentProvider() // Inclure le provider
        };
    }
    ...
    extractContextClues(email) {
        ...
        // Ajouter des indices selon le provider
        const provider = email.provider || this.getCurrentProvider();
        if (provider === 'google') {
            clues += ' gmail google ';
        } else if (provider === 'microsoft') {
            clues += ' outlook microsoft ';
        }
    ...
    escapeRegex(string) {
        return string.replace(/[.*+?^${}()|[\]\\]/g, '\\    saveSettingsToStorage() {
        try {
            localStorage.setItem('categorySettings', JSON.stringify(this');
    }
    ...
    isSpamEmail(email) {
        ...
        // Vérification spéciale Gmail (labelIds)
        if (email.provider === 'google' && email.categories) {
            const hasSpamLabel = email.categories.includes('SPAM');
    ...
    getCurrentUserEmail() {
        try {
            // Essayer depuis le provider actuel d'abord
            const provider = this.getCurrentProvider();
            
            if (provider === 'google' && window.googleAuthService) {
                const googleUser = window.googleAuthService.getCurrentUser?.();
            ...
            if (provider === 'microsoft' && window.authService) {
                const msUser = window.authService.getCurrentUser?.();
                if (msUser && (msUser.email || msUser.userPrincipalName)) {
                    return msUser.email || msUser.userPrincipalName;
                }
            }
    ...
    testEmail(subject, body = '', from = 'test@example.com', expectedCategory = null) {
        ...
        console.log(`Provider: ${testEmail.provider}`);
    ...
    // ================================================
    // DIAGNOSTIC DES CATÉGORIES PROBLÉMATIQUES
    // ================================================
    runDiagnostics() {
        ...
        console.log(`→ Optimisation ${provider} active`);
    ...
    getDebugInfo() {
        return {
            ...
            providerCompatibility: this.providerCompatibility
        };
    }
...
window.testCategoryManager = function() {
    ...
    const provider = window.categoryManager.getCurrentProvider();
    console.log('Provider actuel:', provider);
    
    const tests = [
        { subject: "Newsletter hebdomadaire - Unsubscribe here", expected: "marketing_news" },
        ...
        { subject: "Google Meet invitation - Réunion équipe", expected: "meetings" }
    ];
...
console.log('✅ CategoryManager v20.1 loaded - Gestion robuste Gmail/Outlook');
