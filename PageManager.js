// PageManager.js - Version 10.1 - Optimisée et compacte
class PageManager {
constructor() {
...
init() {
console.log('[PageManager] Initialized v10.1 - Optimisée et compacte');
}
...
return `<button class="category-filter ${this.currentCategory === 'all' ? 'active' : ''}" onclick="window.pageManager.filterByCategory('all')"><span class="filter-icon">📧</span><span class="filter-name">Tous</span><span class="filter-count">${totalEmails}</span></button>${Object.entries(categories).map(([catId, category]) => {const count = categoryCounts[catId] || 0;return count > 0 ? `<button class="category-filter ${this.currentCategory === catId ? 'active' : ''}" onclick="window.pageManager.filterByCategory('${catId}')" style="--category-color: ${category.color}"><span class="filter-icon">${category.icon}</span><span class="filter-name">${category.name}</span><span class="filter-count">${count}</span></button>` : '';}).join('')}${(categoryCounts.other || 0) > 0 ? `<button class="category-filter ${this.currentCategory === 'other' ? 'active' : ''}" onclick="window.pageManager.filterByCategory('other')"><span class="filter-icon">📌</span><span class="filter-name">Autre</span><span class="filter-count">${categoryCounts.other}</span></button>` : ''}`.replace(/>\s+</g, '><');
}
renderEmailsList() {
const emails = window.emailScanner?.getAllEmails() || this.getTemporaryEmails() || [];
...
return `<div class="emails-list">${emails.map(email => this.renderEmailCard(email)).join('')}</div>`.replace(/>\s+</g, '><');
}
...
if (categoryId === 'other') return 'Autre';
const categories = window.categoryManager?.getCategories() || {};
...
console.log('✅ PageManager v10.1 loaded - Optimisée et compacte');
