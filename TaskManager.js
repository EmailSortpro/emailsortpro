...
class PageManager {
    ...
    formatCategoryText(text) {
        // Si le texte fait plus de 15 caractères ou contient un &, on le divise
        if (text.length > 15 || text.includes('&') || (text.includes(' ') && text.length > 10)) {
            // Remplacer & par &<br> pour forcer le retour à la ligne
            if (text.includes('&')) {
                return text.replace(/\s*&\s*/g, '<br>&');
            }
            // Pour les textes longs, couper au premier espace après la moitié
            if (text.includes(' ')) {
            ...
            <button class="status-pill-harmonized-multiline ${this.currentCategory === tab.id ? 'active' : ''}" 
                ...
                <div class="pill-content-row">
                    ...
                    <div class="pill-text-row">
                        <span class="pill-text-harmonized-multiline">${this.formatCategoryText(tab.name)}</span>
                    </div>
                </div>
            </button>
        `).join('');
