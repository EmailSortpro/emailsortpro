// StartScan.js - Version 9.0 - Mise en évidence des catégories pré-sélectionnées avec gestion double auth
...
class MinimalScanModule {
    ...
    async render(container) {
        ...
        try {
            ...
            // Vérifier l'authentification (Microsoft OU Google)
    ...
    async checkAuthentication() {
        // Vérifier si Microsoft OU Google est authentifié
        const microsoftAuth = window.authService?.isAuthenticated() || false;
        const googleAuth = window.googleAuthService?.isAuthenticated() || false;
        ...
        return microsoftAuth || googleAuth;
    }
    ...
    renderNotAuthenticated() {
        // Déterminer quel service d'authentification proposer
            ...
            <div class="minimal-scanner">
                <div class="scanner-card-minimal">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 12px; margin-top: 30px;">
                        ...
                        ${hasGoogleAuth ? `
                            <button class="scan-button-minimal" style="background: linear-gradient(135deg, #4285f4 0%, #34a853 100%);" onclick="window.googleAuthService.login()">
    ...
    async checkServices() {
        // Vérifier que au moins un service d'authentification est disponible
        const microsoftAuth = window.authService?.isAuthenticated() || false;
        const googleAuth = window.googleAuthService?.isAuthenticated() || false;
        
        if (!microsoftAuth && !googleAuth) {
            throw new Error('Authentification requise');
        }
        
        // Si Google est authentifié, vérifier si on a le mailService
        if (googleAuth && !window.mailService) {
            console.warn('[MinimalScan] ⚠️ MailService non disponible pour Google');
        }
    }
